...
public class NoteControllerTest extends ControllerTest {
    ...
    public void getAllNotes_200Ok() throws Exception {
        // given
        CollectionModel<EntityModel<Note>> collectionModel = mockNotes.stream()
        ...
        mockMvc.perform(get(BASE_PATH + NOTE_PATH).contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
    ...
    public void getAllNotesByFolderId_200Ok() throws Exception {
        ...
        when(mockNoteModelAssembler.toCollectionModel(mockNotes)).thenReturn(collectionModel);
        mockMvc.perform(get(BASE_PATH + FOLDER_PATH + "/" + testId + "/notes")
    ...
    public void getAllNotesByTagId_200Ok() throws Exception {
        ...
        when(mockNoteModelAssembler.toCollectionModel(mockNotes)).thenReturn(collectionModel);
        mockMvc.perform(get(BASE_PATH + TAG_PATH + "/" + testId + "/notes")
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.jsonPath("$.size()").value(2))
    ...
    public void getNoteById_200Ok() throws Exception {
        ...
        mockMvc.perform(get(BASE_PATH + NOTE_PATH + "/" + testId).contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void createNoteByFolderId_201Created() throws Exception {
        ...
        Note newNote = Note.builder()
                ...
                .tags(new ArrayList<>())
                .build();
        EntityModel<Note> entityModel = EntityModel.of(newNote,
                linkTo(methodOn(NoteController.class).getNoteById(newId)).withSelfRel());
        String requestBody = mapper.writeValueAsString(new NoteCreationForm("New Note", "This is a new note."));
        ...
        mockMvc.perform(MockMvcRequestBuilders.post(BASE_PATH + FOLDER_PATH + "/" + testId + "/notes")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andDo(print())
    ...
    public void updateNoteById_200Ok() throws Exception {
        ...
        when(mockNoteModelAssembler.toModel(testNote)).thenReturn(entityModel);
        mockMvc.perform(MockMvcRequestBuilders.patch(BASE_PATH + NOTE_PATH + "/" + testId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(requestBody))
                .andDo(print())
                .andExpect(MockMvcResultMatchers.status().isOk())
    ...
    public void deleteNoteById_204NoContent() throws Exception {
        ...
        // when, then
        mockMvc.perform(MockMvcRequestBuilders.delete(BASE_PATH + NOTE_PATH + "/" + testId)
                        .contentType(MediaType.APPLICATION_JSON))
                .andDo(print())
